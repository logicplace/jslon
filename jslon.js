...
function usU(a,v){return String.fromCharCode(parseInt(v,16));}
function unescapeString(string){
	...
	.replace(/\\u([0-9a-fA-F]{4})/g,usU)
	.replace(/\\(.)/g,"\\1")
	;
}

function escapeStr(str,q,esc){
...
function specific(obj,x,opts,depth){
	var spec = opts.specific;
	if(spec && obj.constructor.name == spec.constructor.name && x in spec && spec[x]){
		spec = spec[x];
		for(var k in opts){
			if(!(k in spec) && k != "specific")spec[k] = opts[k]
		}
		return stringifyValue(obj[x],spec,depth);
	}
	return stringifyValue(obj[x],opts,depth);
}
	...
	switch(obj.constructor.name){
		case "Object": {
			...
			for(var x in obj){
				...
				nextIsNewLine = false;
				var q = "";
				if(!("keyQuotes" in options))q = '"'
				else if(options.keyQuotes)q = options.quotes || "";
				if(q == "" && x.match(/^[$_a-zA-Z][$_a-zA-Z0-9]*$/)){
					ret += x+":";
				} else {
					q = (q||'"');
					ret += escapeStr(x,q,options.strEscapes)+":";
				}
				...
				ret += specific(obj,x,options,depth);
				if(options.keyOwnLine){
			...
			if(options.quotes == "'" || options.quotes == '"'){
				return escapeStr(obj,options.quotes,options.strEscapes);
			} else {
				return escapeStr(obj,'"',options.strEscapes);
			}
		} case "Number": {
